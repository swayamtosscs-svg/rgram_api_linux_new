...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (followRequest.following.toString() !== currentUserId) {
      return res.status(403).json({ success: false, message: 'You can only accept follow requests sent to you' });
    }
    ...
    await User.findByIdAndUpdate(followRequest.following, { $inc: { followersCount: 1 } });
