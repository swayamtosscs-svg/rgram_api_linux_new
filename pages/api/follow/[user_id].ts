      ...
      if (existingFollow) {
        ...
        if (userToFollow.isPrivate) {
          ...
          try {
            ...
            // Fallback if enum mismatch on server
            const isEnumError = typeof notifErr?.message === 'string' && notifErr.message.includes('is not a valid enum value for path `type`');
            if (isEnumError) {
              try {
                await Notification.create({
                  recipient: user_id,
                ...
                const isEnumError2 = typeof friendErr?.message === 'string' && friendErr.message.includes('is not a valid enum value for path `type`');
                if (isEnumError2) {
                  await Notification.create({
                    recipient: user_id,
                    sender: followerId,
                    type: 'follow',
                    content: `${followerUser?.fullName || followerUser?.username} wants to follow you`
                  });
                } else {
                  throw friendErr;
                }
